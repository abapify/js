...
export function parseDiscoveryXml(xml: string): DiscoveryData {
  ...
  const workspacesArray = Array.isArray(workspacesRaw) ? workspacesRaw : [workspacesRaw];
  
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const workspaces: WorkspaceData[] = workspacesArray.map((ws: Record<string, any>) => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const collections: CollectionData[] = collectionsArray.map((coll: Record<string, any>) => {
      // Parse accepts
      const acceptsRaw = coll['app:accept'];
      const accepts = acceptsRaw
        ? (Array.isArray(acceptsRaw) ? acceptsRaw : [acceptsRaw])
        : [];
      
      // Parse category
      const categoryRaw = coll['atom:category'];
      const category = categoryRaw
        ? { term: categoryRaw['@_term'] || '', scheme: categoryRaw['@_scheme'] || '' }
        : { term: '', scheme: '' };
      
      // Parse template links
      const templateLinksContainer = coll['adtcomp:templateLinks'];
      const templateLinksRaw = templateLinksContainer?.['adtcomp:templateLink'];
      const templateLinks = templateLinksRaw
        ? (Array.isArray(templateLinksRaw) ? templateLinksRaw : [templateLinksRaw])
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
