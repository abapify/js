...
describe('Wave 2 generic handlers', () => {
  ...
  it('emits formatVersion "1" + header for a sample type', async () => {
    ...
    const files = await handler.serialize({
      name: 'Z_TEST', description: 'Enhancement', originalLanguage: 'EN',
    } as any);
    const json = JSON.parse(files.find((f) => f.path.endsWith('.json'))!.content);
  ...
  it('emits .json metadata file with correct type in filename', async () => {
    ...
    const files = await handler.serialize({
      name: 'Z_CHDO', description: 'Change doc', originalLanguage: 'DE',
    } as any);
  ...
  it('round-trips via fromAbapGit', async () => {
    ...
    const files = await handler.serialize({
      name: 'Z_NROB', description: 'Number range', originalLanguage: 'EN',
    } as any);
    const json = JSON.parse(files.find((f) => f.path.endsWith('.json'))!.content);
