...
export default defineConfig({
  ...
  generators: [
    ...
    interfaces({
      ...
      generateRootType: true,  // Generate root schema type (e.g., PackagesV1Schema)
      addJsDoc: true,
    }),
  ],
  ...
  afterAll({ sources }) {
    ...
    const sapSchemas = allSchemas.filter((s: string) => s.startsWith('sap/')).map((s: string) => s.replace('sap/', ''));
    const customSchemas = allSchemas.filter((s: string) => s.startsWith('custom/')).map((s: string) => s.replace('custom/', ''));
    ...
    const sapSchemasIndex = [
      ...
      ' */',
      '',
      ...sapSchemas.map((s: string) => `export { default as ${s.replace(/-/g, '')} } from './${s}';`),
      '',
    ].join('\n');
    ...
    const customSchemasIndex = [
      ...
      ' */',
      '',
      ...customSchemas.map((s: string) => `export { default as ${s.replace(/-/g, '')} } from './${s}';`),
      '',
    ].join('\n');
    ...
    const toRootTypeName = (name: string) => deriveRootTypeName(name) ?? `${name}Schema`;
    ...
    const targetSapSchemas = targetSchemas.filter((s: string) => s.startsWith('sap/')).map((s: string) => s.replace('sap/', ''));
    const targetCustomSchemas = targetSchemas.filter((s: string) => s.startsWith('custom/')).map((s: string) => s.replace('custom/', ''));
    ...
    for (const schemaName of targetSapSchemas) {
      ...
      sapTypeImportLines.push(`import type { ${rootTypeName} } from './types/sap/${schemaName}.types';`);
      typedLines.push(`import ${importName} from './schemas/sap/${schemaName}';`);
      typedLines.push(`export const ${exportName} = typed<${rootTypeName}>(${importName} as SchemaLike);`);
    }
    ...
    for (const schemaName of targetCustomSchemas) {
      ...
      customTypeImportLines.push(`import type { ${rootTypeName} } from './types/custom/${schemaName}.types';`);
