...
describe('emitLocalClasses — localsImp', () => {
  it('uses the expected SAP kernel APIs', () => {
    ...
    // Cloud-approved URL encoding lives on `cl_web_http_utility`; the
    // legacy `cl_http_utility` is not on the Steampunk whitelist.
    expect(localsImp).toContain('cl_web_http_utility=>escape_url');
    expect(localsImp).not.toContain('cl_http_utility=>');
    // `set_binary_body` was renamed to `set_binary` (the former does not
    // exist on `if_web_http_request` in ABAP cloud).
    expect(localsImp).not.toContain('set_binary_body');
    expect(localsImp).toContain('request->set_binary(');
    // `DESCRIBE FIELD` is disallowed on cloud; RTTI is used instead.
    expect(localsImp).not.toContain('DESCRIBE FIELD');
    expect(localsImp).toContain('cl_abap_typedescr=>describe_by_data');
