 ...
 * Maps ADK Domain objects to abapGit XML structure
 */

import type { Domain } from '@abapify/adk';
import type { DdicFixedValueType } from '@abapify/adt-schemas';
import { Dd01vTableSchema, Dd07vTabSchema } from './schema.js';
import type { Dd01vTable, Dd07vEntry, Dd07vTab } from './types.js';
import { build } from 'ts-xml';
import { wrapTextFields } from '../../lib/create-serializer.js';

/**
 * Serialize ADK Domain to abapGit XML
 *
 * Domains have TWO root elements under <asx:values>:
 * - DD01V (domain header)
 * - DD07V_TAB (fixed values table, optional)
 */
export function serializeDomain(doma: Domain): string {
  const data = doma.getData();
