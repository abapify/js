 ...
 * ```
 */
export function inferredTypes(
  options: InferredTypesOptions = {},
): GeneratorPlugin {
  const { inferSchemaImport = 'ts-xsd', typeNamePattern = '{Name}' } = options;
  ...
  return {
    ...
    transform(ctx: TransformContext): GeneratedFile[] {
      const { schema } = ctx;
      const typeName = typeNamePattern.replace(
        '{Name}',
