...
function isUnavailable(err: unknown): err is HttpError {
  ...
  if (typeof e.status === 'number' && [401, 403, 404, 405, 406, 501].includes(e.status)) {
    return true;
  }
...
async function probeGet(path: string, accept = 'application/xml'): Promise<ProbeResult> {
...
describeReal('Backfill sweep — synthetic fixtures', () => {
  ...
  it('E04 STRUST — /sap/bc/adt/system/security/pses', async () => {
    const probe = await probeGet('/sap/bc/adt/system/security/pses', 'application/atom+xml');
    if (!probe.ok) {
    ...
    captureFixture(
      ...
      'system/security/real-pse-list.xml',
    );
  ...
  it('E07 gCTS — GET /sap/bc/cts_abapvcs/repository', async () => {
    const probe = await probeGet('/sap/bc/cts_abapvcs/repository', 'application/json');
    if (!probe.ok) {
      // eslint-disable-next-line no-console
      console.warn(`[E07 gCTS] repositories list unavailable on TRL: ${probe.error}`);
      return;
    }
    captureFixture(
  ...
  it('E07 gCTS — GET /sap/bc/cts_abapvcs/config', async () => {
    ...
    const probe = await probeGet('/sap/bc/cts_abapvcs/config', 'application/json');
    if (!probe.ok) {
      // eslint-disable-next-line no-console
      console.warn(`[E07 gCTS] config endpoint unavailable on TRL: ${probe.error}`);
      return;
    }
    captureFixture(
      probe.body as Record<string, unknown>,
