 ...
 */
type ExtractRequest<T> = ExtractDescriptor<T> extends RestEndpointDescriptor<
  infer TMethod,
  infer TPath,
  infer TBody,
  infer _TResponses
>
  ? {
      ...
      query?: ExtractDescriptor<T> extends { query?: infer Q } ? Q : undefined;
 ...
 */
type ExtractResponse<T> = ExtractDescriptor<T> extends RestEndpointDescriptor
 ...
 */
type ExtractBody<T> = ExtractDescriptor<T> extends RestEndpointDescriptor<
  infer _TMethod,
  infer _TPath,
  infer TBody,
  infer _TResponses
...
> {
  ...
  getDescriptor(): ExtractDescriptor<TContract> {
    return this.contract(...this.getContractParams()) as ExtractDescriptor<TContract>;
  }
}
 ...
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function runTypedScenario<T extends (...args: any[]) => RestEndpointDescriptor>(
  scenario: TypedContractScenario<T>
): void {
  describe(scenario.name, () => {
    ...
    describe('response', () => {
      it('parses fixture and passes response assertions', async () => {
        ...
        const client = createClient({ operation: scenario.contract }, {
          baseUrl: '',
          adapter,
        });
        
        // Call the operation
        const response = await (client.operation as unknown as (...args: unknown[]) => Promise<unknown>)(...scenario.getContractParams());
