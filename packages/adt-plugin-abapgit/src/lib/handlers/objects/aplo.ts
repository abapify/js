...
export const applicationLogObjectHandler = createHandler<AploLike, typeof bdef>(
  'APLO',
  {
    schema: bdef,
    version: 'v1.0.0',
    serializer: 'LCL_OBJECT_APLO',
    serializer_version: 'v1.0.0',
    toAbapGit: (obj) => ({
      SKEY: { TYPE: 'APLO', NAME: String(obj?.name ?? '').toUpperCase() },
    }),
    ...
    async serialize(object, ctx): Promise<SerializedFile[]> {
      ...
      return [
        ctx.createFile(
          `${ctx.getObjectName(object)}.aplo.json`,
