...
export const cdboHandler = createHandler<CdboLike, typeof bdef>('CDBO', {
  schema: bdef,
  version: 'v1.0.0',
  serializer: 'LCL_OBJECT_CDBO',
  serializer_version: 'v1.0.0',
  toAbapGit: (obj) => ({
    SKEY: { TYPE: 'CDBO', NAME: String(obj?.name ?? '').toUpperCase() },
  }),
  ...
  async serialize(object, ctx): Promise<SerializedFile[]> {
    ...
    return [
      ctx.createFile(
        `${ctx.getObjectName(object)}.cdbo.json`,
