...
export const bgqcHandler = createHandler<BgqcLike, typeof bdef>('BGQC', {
  schema: bdef,
  version: 'v1.0.0',
  serializer: 'LCL_OBJECT_BGQC',
  serializer_version: 'v1.0.0',
  toAbapGit: (obj) => ({
    SKEY: { TYPE: 'BGQC', NAME: String(obj?.name ?? '').toUpperCase() },
  }),
  ...
  async serialize(object, ctx): Promise<SerializedFile[]> {
    ...
    return [
      ctx.createFile(
        `${ctx.getObjectName(object)}.bgqc.json`,
