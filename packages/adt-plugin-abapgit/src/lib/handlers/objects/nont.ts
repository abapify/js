...
export const objectTypeNodeHandler = createHandler<NontLike, typeof bdef>(
  'NONT',
  {
    schema: bdef,
    version: 'v1.0.0',
    serializer: 'LCL_OBJECT_NONT',
    serializer_version: 'v1.0.0',
    toAbapGit: (obj) => ({
      SKEY: { TYPE: 'NONT', NAME: String(obj?.name ?? '').toUpperCase() },
    }),
    ...
    async serialize(object, ctx): Promise<SerializedFile[]> {
      ...
      return [
        ctx.createFile(
          `${ctx.getObjectName(object)}.nont.json`,
