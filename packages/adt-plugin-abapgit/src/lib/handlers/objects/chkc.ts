...
export const checkCategoryHandler = createHandler<ChkcLike, typeof bdef>(
  'CHKC',
  {
    schema: bdef,
    version: 'v1.0.0',
    serializer: 'LCL_OBJECT_CHKC',
    serializer_version: 'v1.0.0',
    toAbapGit: (obj) => ({
      SKEY: { TYPE: 'CHKC', NAME: String(obj?.name ?? '').toUpperCase() },
    }),
    ...
    async serialize(object, ctx): Promise<SerializedFile[]> {
      ...
      return [
        ctx.createFile(
          `${ctx.getObjectName(object)}.chkc.json`,
