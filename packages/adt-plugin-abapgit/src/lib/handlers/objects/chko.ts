...
export const checkObjectHandler = createHandler<ChkoLike, typeof bdef>('CHKO', {
  schema: bdef,
  version: 'v1.0.0',
  serializer: 'LCL_OBJECT_CHKO',
  serializer_version: 'v1.0.0',
  toAbapGit: (obj) => ({
    SKEY: { TYPE: 'CHKO', NAME: String(obj?.name ?? '').toUpperCase() },
  }),
  ...
  async serialize(object, ctx): Promise<SerializedFile[]> {
    ...
    return [
      ctx.createFile(
        `${ctx.getObjectName(object)}.chko.json`,
