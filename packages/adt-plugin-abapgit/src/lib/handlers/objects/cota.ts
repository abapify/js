...
export const communicationTargetHandler = createHandler<CotaLike, typeof bdef>(
  'COTA',
  {
    schema: bdef,
    version: 'v1.0.0',
    serializer: 'LCL_OBJECT_COTA',
    serializer_version: 'v1.0.0',
    toAbapGit: (obj) => ({
      SKEY: { TYPE: 'COTA', NAME: String(obj?.name ?? '').toUpperCase() },
    }),
    ...
    async serialize(object, ctx): Promise<SerializedFile[]> {
      ...
      return [
        ctx.createFile(
          `${ctx.getObjectName(object)}.cota.json`,
